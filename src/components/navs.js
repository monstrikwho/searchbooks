...
class Navs extends React.Component {
  ...
  handleButtonClick = () => {
    var inputValue = this.state.inputValue
      .replace(/^\s+/g, "")
      .replace(/\s+$/g, ""); // удаляем пробелы сначала и в конце
        ...
        <Container>
          <div className="navbar-action">
            <Link
              ...
              onClick={() => {
                this.props.renameNavbar(
                  "#НАЙДИКНИГУ",
                  this.props.pageName === "read" ? "search" : "home"
                );
            ...
            >
              {this.props.pageName === "home" ? "" : <BackDoorSVG />}
            </Link>
            <Navbar.Brand>{this.props.navBarTitle}</Navbar.Brand>
          </div>
          {this.props.pageName === "read" ? (
            ""
          ) : (
